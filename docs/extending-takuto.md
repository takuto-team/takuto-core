...
your Takuto deployment without forking the upstream image.

Takuto uses a **three-tier extension model**:

| Tier | Mechanism | When to use |
|---|---|---|
| **Baked** | This Dockerfile | Required for advertised Takuto features |
| **Provisioning** | `[provisioning].install_commands` in `config.toml` | Admin preferences — single-binary tools, version pins |
| **Custom image** | Custom Dockerfile `FROM takuto:latest` | Specialized cases — system packages, multi-path installs |
...
- **Baked** tools live in the official Takuto image. The bake set is small and
  load-bearing: every binary here exists because at least one advertised
  Takuto feature needs it. Adding a tool to the bake increases image size
  for every deployment forever, so the bar is high.
- **Provisioning** tools live in the shared `takuto-tools` Docker volume.
  ...
  editor / run-command Takuto spawns, and `/opt/takuto-tools/bin` is FIRST
...
- **Removed** tools used to be baked but were moved out because the
  cost/benefit didn't justify shipping them to every deployment. The Takuto
...
| Add system packages (apt, libraries) | Custom Dockerfile `FROM takuto:latest` | `apt-get install kubectl awscli` |
...
| Replace the Takuto binary itself | Custom Dockerfile (rebuild) | Local development build |

## Mechanism 1 — `[provisioning].install_commands`

### What it does

At every takuto startup, the entrypoint runs the SHA-gated install pass:

1. Compute the canonical SHA-256 of the (JSON-encoded) `install_commands` list.
2. Read `/opt/takuto-tools/.provisioning-sha`.
3. If the two SHAs match → **skip** the install pass. This is the fast path
   and runs every boot when nothing has changed.
4. If they differ (or the file is missing): export `TAKUTO_TOOLS_BIN=/opt/takuto-tools/bin`,
...
- Takuto container: `/opt/takuto-tools/bin` (read-write — only the
  entrypoint writes here).
- Every spawned container (workers, editors, run-commands): the same path,
  but **read-only**, via the `takuto-tools` named Docker volume.
- `$PATH`: prepended with `/opt/takuto-tools/bin`, so anything in the
...
- `$TAKUTO_TOOLS_BIN` is exported to `/opt/takuto-tools/bin` — use it as
  the install target.
- Commands run as **root**. To run as the `takuto` user (e.g. `mise install`
  populating a user-scoped store), wrap with `runuser -u takuto -- …`.
...
revert: remove the line, restart, and `rm /opt/takuto-tools/bin/claude`.
...
Set `MYCO_TOKEN` in `takuto.env` (mounted into the takuto container) so
...
## Mechanism 2 — Custom Dockerfile (`FROM takuto:latest`)
...
# Drop back to takuto for the runtime entrypoint.
USER takuto
```
...
services:
  takuto:
    # image: ghcr.io/takuto-team/takuto-core:latest   # ← comment out
    build:
      context: .
      dockerfile: my-takuto.Dockerfile
```

Run: `docker compose build takuto && docker compose up -d`.

### Why use this over `[provisioning]`?

- System packages need apt + root + dependencies that touch multiple
  directories (`/etc`, `/usr/share`, `/var/lib`). The `takuto-tools`
  volume only mounts `/opt/takuto-tools/bin` — a single-directory write
...
- Apt installs are also more disk-efficient when they happen at image
  ...
  deployment).
- If you need the tool available even when the `takuto-tools` volume is
...
Takuto reads `GH_HOST`, `MY_API_TOKEN`, and any other env from this file
...
ONLY affect the takuto service itself.
...
```bash
docker exec takuto-core-takuto-1 ls /opt/takuto-tools/bin/
```

If the tool isn't there, check the boot log for `[takuto-provisioning]
cmd N/M: WARN` lines. Common causes:

- Network error during install (curl failed).
- `$TAKUTO_TOOLS_BIN` permissions wrong (try
  `docker exec --user root takuto-core-takuto-1 ls -la /opt/takuto-tools/`).
- The install command requires a system package not in the takuto image —
...
`/opt/takuto-tools/bin` should be first. If it isn't, the worker image
diverged from the takuto image (typically because someone overrode
`TAKUTO_REGISTRY_IMAGE`).

### "I want to roll back to takuto's pinned version of a baked tool"

Just remove the override command from `[provisioning]` and restart:

```bash
docker exec --user root takuto-core-takuto-1 \
    rm /opt/takuto-tools/bin/claude   # or whichever tool
docker compose restart takuto
```
...
`[provisioning]` config block and the `takuto-tools` volume are the
underlying mechanism that won't change. Future Takuto versions will
remain backward-compatible with `[provisioning]` configurations written
today.

## See also

- `README.md` — Project overview and quickstart (includes the "Extending Takuto" quickstart section).
