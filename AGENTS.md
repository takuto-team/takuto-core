...
- **`general`**: `dry_mode`, `poll_interval_secs`, **`pause_jira_polling_on_startup`** (when **`true`**, Jira polling starts paused — same as dashboard **Pause polling**; **Resume polling** / **`POST /api/polling/resume`** still applies; restart re-reads this key from disk), **`max_concurrent_workflows`** (semaphore for concurrent install/agent work), **`max_active_workflows`** (**`0`** = use **`max_concurrent_workflows`**; Jira poller counts **every visible dashboard row** still in the workflow map), `log_level`, `worker_image`, optional **`ticket_workflow_steps_file`**, **`review_workflow_steps_file`**, **`merge_base_workflow_steps_file`**
