...
- **`general`**: `dry_mode`, `poll_interval_secs`, **`max_concurrent_workflows`** (semaphore for concurrent install/agent work), **`max_active_workflows`** (**`0`** = use **`max_concurrent_workflows`**; Jira poller counts **every visible dashboard row** still in the workflow map), `log_level`, `worker_image`, optional **`ticket_workflow_steps_file`**, **`review_workflow_steps_file`**, **`merge_base_workflow_steps_file`**
...
`crates/maestro-core/src/jira/poller.rs`: on an interval, if `project_keys` non-empty, lists **To Do** tickets (via `JiraClient` / `acli`), skips keys that already exist in the workflow map, and only starts new workflows when **`dashboard_workflow_count`** is less than **`effective_max_active_workflows()`** (**`[general] max_active_workflows`**, or **`max_concurrent_workflows`** when **`max_active_workflows`** is **`0`**). **Every row still on the dashboard counts** — **Done**, **Paused**, **Stopped**, **Error**, and in-progress — until **Mark as Done** or **Delete** removes it from the map. **Restored** workflows are in the map before the first poll, so they consume slots. Uses `cancel_token` for shutdown. When the dashboard sets **polling paused** (`AppState.polling_paused`), the poller still waits on the interval but **skips** `poll_once` (including the initial poll on startup if already paused). Polling pause is not persisted across process restarts; workflow snapshot resume is (see **How the binary starts**).
