...
6. Constructs `JiraPoller` with a shared `CancellationToken` and **`Arc<AtomicBool>`** polling pause flag (shared with `AppState`). The flag is **`false`** by default; set **`[general] pause_jira_polling_on_startup = true`** in **`config.toml`** to start in the same paused state as the dashboard **Pause polling** action, then `build_router`.
...
| POST | `/api/workflows/start-manual` | JSON **`{ "ticket_key", "ticket_summary" }`** — start the main ticket pipeline for that key (**409** if a workflow exists, Jira projects unset, or **`max_concurrent_manual_workflows`** reached). Returns **`{ "workflow_id", "ticket_key" }`**. |
