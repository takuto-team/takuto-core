...
| Method | Path | Notes |
|--------|------|--------|
| GET | `/api/workflows` | List summaries (includes `id` = workflow UUID, `ticket_key`, **`pr_url`**, **`progress_percent`** (0–100, step-based from config + `steps_log`), **`progress_steps_total`** (estimated step count for the current phase — same basis as dashboard segment count / `k/N` label), **`started_manually`**, **`counts_toward_manual_cap`**, action flags such as **`can_delete`**, etc.); sorted by **`started_at`** ascending (oldest first — matches **first-load** dashboard order; the UI then **preserves** that order on updates and only **appends** new keys). Dashboard **Show PR** uses **`pr_url`** when set. |
