...
Source: 2026-05-24 typed-errors architecture spec (Part A is binding — see `lore/audits/2026-05-24-typed-errors-spec.md`). Executes A.1 row 6: carve `TakutoError::Git(String)` into `GitError`.

## 1. Module layout — `git/error.rs` + `mod.rs` re-export

`git/` is already a directory (`pr.rs`, `remote.rs`, `worktree.rs`, `worktree_remove.rs`). Add `git/error.rs`; declare via `pub mod error; pub use error::GitError;` in `mod.rs`. The 21 `TakutoError::Git(format!(...))` call sites span 5 files — `actions/{real,dry_run,gh_github}.rs` and `git/worktree_remove.rs` and `workflow/engine/bootstrap.rs` — so the canonical home is `git/error.rs` because the variants are "things that fail when invoking git or `gh` against a worktree", which is the `git/` subsystem's domain plus the side-effecting `actions/` impls.

## 2. `GitError` definition — 14 variants

Lands at `crates/takuto-core/src/git/error.rs`. Operation clusters:
...
- `std::io::Error` → `#[source]` on `WorktreeDirRemoveFailed`. Avoids collision with `TakutoError::Io(#[from] std::io::Error)` envelope (same rationale as GitHubAppError).

## 4. Migration plan — 2 commits + lock-in (in C1)

- **C1** `refactor(git): C1 — land GitError + envelope + rename Git → GitStr` — define `GitError`, add `TakutoError::Git(#[from] GitError)`, rename `Git(String)` → `GitStr(String)` with `#[deprecated]`, mechanically sed all 21 sites to `::GitStr(`, gated under function-level `#[allow(deprecated)]` on the 10 enclosing fns (`create_worktree`×2, `delete_local_branch`×2, `fetch_gh_user`, `apply_git_identity_from_gh`, `gh_request_self_pr_reviewer`, `remove_git_worktree`, `clear_worktree_path_for_recreate`, `bootstrap_new_workflow`). Add 2 lock-in tests (Display + From impl, both with `cases.len() == 14` drift assertion).
- **C2** `refactor(git): C2 — migrate git/, actions/, bootstrap to GitError` — 19 sites become typed variants, 2 collapse to direct propagation (`bootstrap.rs:553,658` — both wrap an inner `TakutoError` with a zero-info `"<step> error: {e}"` prefix; `step_log.fail()` keeps the formatted message inline, the Err return drops the prefix), attrs from C1 removed.
...
- [x] Zero `TakutoError::Git(` / `TakutoError::GitStr(` constructions remain under `crates/takuto-core/src/` (one matches!() pattern in the test assertion remains).
- [x] `TakutoError::GitStr(String)` retained as `#[deprecated]` shim — removed by the post-§8 #2 cleanup PR.
...
- **NO** changes to `process::run_command` / `run_shell_command*` / `ProcessHandle` — these still return `TakutoError` with `Io` / `Command` / `Timeout` envelopes.
