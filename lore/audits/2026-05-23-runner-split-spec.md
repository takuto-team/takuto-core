...
Source: 2026-05-21 clean-code audit §9 ("Per-layer cut plans") — first target on the §8 priority #3 list ("Decompose the three Rust god modules"). Current state: `crates/takuto-core/src/container/runner.rs` = 1,513 LOC (audit §1 worst-offender #4).
...
Produce these 7 files under `crates/takuto-core/src/container/`:
...
- `dind_paths.rs` — `translate_path_for_dind`, `translate_path_for_dind_inner`, `is_remote_docker_daemon`, plus the `DIND_DATA_PREFIX_ENV` / `TAKUTO_DATA_DIR_HOST_PREFIX` consts.
...
- NO `TakutoError` rework (that is §8 item #2 — separate task).
...
- [ ] Public re-export surface of `crate::container::*` is identical before/after. Verify by grepping for `container::` and `super::` references across `crates/takuto-cli/`, `crates/takuto-web/`, and the rest of `crates/takuto-core/` — every existing path resolves.
