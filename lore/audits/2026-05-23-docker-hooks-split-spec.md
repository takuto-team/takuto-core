...
Source: 2026-05-21 clean-code audit §9 ("Per-layer cut plans") — third target on the §8 priority #3 list ("Decompose the three Rust god modules"). Current state: `crates/maestro-core/src/docker_hooks.rs` = 1,216 LOC (audit §1 worst-offender #6).

## Goal

Split `docker_hooks.rs` into focused sub-modules under `docker_hooks/` per the audit's cut plan, with zero behaviour change and an identical public surface (every `crate::docker_hooks::*` and `maestro_core::docker_hooks::*` path keeps resolving).

## Scope (in)

Produce these 7 files under `crates/maestro-core/src/docker_hooks/`:
...
- `status_types.rs` — the `SystemStatus` shape and its siblings: `SystemStatus`, `GitHubStatus`, `ProviderStatus`, `TicketingStatus`, `StructuredWarning` (incl. `critical` / `warning` ctors and the `pub fn info` escape hatch used by `maestro-web::config_agent`), `PreflightResult`, `impl SystemStatus { has_critical }`, `impl Default for SystemStatus`. Today at lines 292-297 + 299-448.
...
- NO `MaestroError` changes (that is §8 item #2 — separate task).
- NO `ExternalActions` trait changes.
- NO changes to `maestro-web/src/state.rs`, `maestro-cli/src/main.rs`, or any other caller — every existing `docker_hooks::*` import path must continue to resolve unchanged.
- NO modification to the `pub mod docker_hooks;` line in `crates/maestro-core/src/lib.rs:25`.
...
- [ ] Public re-export surface of `crate::docker_hooks::*` is identical before/after. Verify by grepping `docker_hooks::` across `crates/maestro-core/`, `crates/maestro-web/`, and `crates/maestro-cli/` — every existing path resolves (the 17 references found at spec time, covering `SystemStatus`, `StructuredWarning`, `run_hook_commands`, `collect_system_status`, `collect_system_status_with_db`, `check_config_dir_writable`).
...
- [ ] `StructuredWarning::info` stays `pub` (used by `maestro-web/src/routes/config_agent.rs:330` outside the crate). `critical` / `warning` stay crate-private as today.
...
- [ ] No `MaestroError` variant changes, no new `ExternalActions` trait methods. The deprecated `preflight()` still returns `Err(MaestroError::Config(_))` via the same `status.has_critical()` branch.
...
2. **`check_config_dir_writable` placement.** It is logically a "config dir check" but its only caller is `maestro-web/src/routes/config_agent.rs:313` and its co-located tests live in `system_status_tests`. Putting it next to `collect_system_status` (in `status.rs`) keeps the tests undisrupted; a sibling `config_writable.rs` would split the tests away from the fn and add a fourth small file.
