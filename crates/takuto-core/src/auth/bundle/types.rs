...
pub const WORKER_SECRETS_MOUNTPOINT: &str = "/run/takuto-secrets";

/// File names inside the secrets directory. The worker entrypoint reads each
/// of these into the matching env var when `TAKUTO_AUTH_BUNDLE=1` is set.
...
/// the takuto `data_dir`. We deliberately don't expose this as a config
...
pub struct WorkerSecretsBundle {
    ...
    /// `CLAUDE_CODE_OAUTH_TOKEN` / `CURSOR_API_KEY` from `/etc/takuto/env`).
    ...
    /// `Some` for `TokenSource::UserPat` and the App identity ("takuto-bot[bot]"),
    ...
    /// `TAKUTO_AUTH_BUNDLE=1` (a discriminator the entrypoint switches on).
    ...
    /// `/home/takuto/.config/opencode/` so OpenCode reads its
