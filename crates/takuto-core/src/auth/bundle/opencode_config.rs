...
//!
//! Takuto's OpenCode role in v1 is self-hosted-only (LM Studio / Ollama /
...
/// bind-mounts the parent dir at `/home/takuto/.config/opencode:ro` in
...
pub(super) fn write_opencode_config(
    ...
    // FIRST slash. Takuto always names the provider `self_hosted`, so the
    ...
    provider_entry.insert(
        "name".to_string(),
        Value::String("Self-hosted (Takuto)".to_string()),
    );
    provider_entry.insert(
        "options".to_string(),
        json!({
