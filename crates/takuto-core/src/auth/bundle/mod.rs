...
//! read-only into the worker at `/run/takuto-secrets/`. The worker
...
mod tests {
    ...
    async fn build_emits_anthropic_base_url_env_for_claude() {
        ...
        // TAKUTO_AUTH_BUNDLE is the worker entrypoint's discriminator.
        assert!(
            bundle
                .extra_env
                .iter()
                .any(|(k, v)| k == "TAKUTO_AUTH_BUNDLE" && v == "1")
        );
    }
    ...
    ///   1. Bind-mount the bundle's host_dir RO at /run/takuto-secrets, AND
    ...
    fn apply_secrets_bundle_to_args_mounts_ro_and_copies_extra_env_only() {
        ...
        let bundle = WorkerSecretsBundle {
            ...
            extra_args: vec![],
            extra_env: vec![
                ("TAKUTO_AUTH_BUNDLE".into(), "1".into()),
        ...
        assert!(has_env("TAKUTO_AUTH_BUNDLE", "1"));
    ...
    fn debug_does_not_leak_token_bytes() {
        ...
        let bundle = WorkerSecretsBundle {
            ...
            extra_args: vec![],
            extra_env: vec![("TAKUTO_AUTH_BUNDLE".into(), "1".into())],
            opencode_config_dir: None,
            _temp_dir: dir,
        };
        let s = format!("{bundle:?}");
    ...
    async fn build_opencode_emits_config_dir_with_bearer_baseurl_model() {
        ...
        // Spec §2.1: OpenCode workflows do NOT receive `/run/takuto-secrets/opencode`.
        assert!(
            bundle.provider_secret_file.is_none(),
            "OpenCode bundles must not write /run/takuto-secrets/opencode \
    ...
    fn bundle_temp_dir_survives_clone_drop_until_last_arc_released() {
        let bundle = Arc::new(WorkerSecretsBundle::for_tests(
            ...
            vec![("TAKUTO_AUTH_BUNDLE".into(), "1".into())],
        ));
        let host_dir = bundle.host_dir().to_path_buf();
        assert!(
            host_dir.exists(),
