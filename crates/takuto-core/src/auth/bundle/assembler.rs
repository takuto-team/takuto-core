...
/// read-only into the worker at `/home/takuto/.config/opencode/` so
...
pub async fn build(
    ...
    // (per arch doc §3.3) rather than the process `/tmp`. The takuto
    ...
    // file instead of `/run/takuto-secrets/opencode`, so the bundle skips
    ...
    // bind-mounts the parent dir at /home/takuto/.config/opencode:ro so
    ...
    let mut extra_env: Vec<(String, String)> =
        vec![("TAKUTO_AUTH_BUNDLE".to_string(), "1".to_string())];
