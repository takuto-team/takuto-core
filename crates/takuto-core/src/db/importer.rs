...
//!   - no legacy `{data_dir}/takuto.db` file exists,
...
/// True when `{data_dir}/takuto.db` exists on disk.
pub fn legacy_sqlite_exists(data_dir: &Path) -> bool {
    data_dir.join("takuto.db").is_file()
}
...
pub async fn import_from_sqlite(
    ...
    let source_path = data_dir.join("takuto.db");
...
mod tests {
    ...
    fn legacy_sqlite_exists_reads_data_dir() {
        ...
        std::fs::write(tmp.path().join("takuto.db"), b"").unwrap();
        assert!(legacy_sqlite_exists(tmp.path()));
    }
