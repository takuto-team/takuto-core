...
pub enum PoolError {
    ...
    /// `sqlite://{data_dir}/takuto.db` BEFORE handing the URL to
...
/// the existing takuto.db so the importer can read it alongside the
/// sqlx-driven writes.
///
...
mod tests {
    ...
    fn redact_password_replaces_password_component() {
        ...
        assert_eq!(
            redact_password_in_url("mysql://root:hunter2@db:3306/takuto"),
            "mysql://root:****@db:3306/takuto"
        );
    }
    ...
    fn redact_password_leaves_passwordless_urls_alone() {
        ...
        assert_eq!(
            redact_password_in_url("sqlite:///var/lib/takuto.db"),
            "sqlite:///var/lib/takuto.db"
        );
