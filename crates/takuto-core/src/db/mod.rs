...
//!
//! The database file lives at `{data_dir}/takuto.db`. Schema migrations run
...
pub mod user_repo_polling_settings;
...
pub struct Database {
    ...
    /// - `TAKUTO_SECRET_KEY` is unset AND
    ...
    /// fixtures. Stored so `takuto keys reset` can rewrite the file in place.
...
impl Database {
    /// Open (or create) the database at `{data_dir}/takuto.db` and run migrations.
    ///
    ...
    pub fn open(data_dir: &Path, allow_auto_generate_secret_key: bool) -> Result<Self> {
        ...
        let db_path = data_dir.join("takuto.db");
    ...
    ///
    /// Resolution:
    ///   - empty / omitted `connection` → SQLite at `{data_dir}/takuto.db`
    ...
    pub async fn connect(
        ...
        // `{data_dir}/takuto.db`, and the target not already carrying
        ...
        if db_config.import_from_sqlite && importer::legacy_sqlite_exists(data_dir) {
            ...
            if !already {
                ...
                tracing::info!(
                    ...
                    source = %data_dir.join("takuto.db").display(),
                    rows = copied,
...
mod connect_tests {
    ...
    async fn empty_connection_uses_default_sqlite() {
        ...
        assert!(
            tmp.path().join("takuto.db").exists(),
            "default path must materialise the takuto.db file"
        );
    }
    ...
    async fn unreachable_backend_with_fail_fast_propagates() {
        ...
        let cfg = DatabaseConfig {
            connection: "postgres://takuto:nope@127.0.0.1:1/nope".to_string(),
            fail_fast: true,
    ...
    /// returns a working SQLite Database pointed at `{data_dir}/takuto.db`.
    #[tokio::test]
