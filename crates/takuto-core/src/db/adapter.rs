...
/// every column type Takuto ever stores. Null-of-type variants exist where
...
/// Adapter-layer error. Sits below `TakutoError` (the crate-wide envelope)
...
/// `MySqlRow`. Provides typed `get_*` accessors for every type Takuto
/// stores.
///
