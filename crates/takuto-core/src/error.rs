...
//! `TakutoError` — the workspace's outermost error envelope.
//!
...
pub enum TakutoError {
    ...
    /// inside `crates/takuto-core/src/git/`,
    /// `crates/takuto-core/src/actions/{real,dry_run,gh_github}.rs`, and
    ...
    /// `crates/takuto-core/src/{cursor,codex,opencode}/session.rs` and
    ...
    /// `crates/takuto-core/src/{auth,db/users,db/credentials}.rs` and
    /// `crates/takuto-web/src/{auth.rs, routes/{auth,admin}.rs}` via
    /// `AuthError::Variant` then `?`-propagated through this `#[from]`.
    #[error(transparent)]
    Auth(#[from] AuthError),

    /// Typed config subsystem error envelope. Produced inside
    /// `crates/takuto-core/src/config/`, `auth/{master_key,seal,bundle}.rs`,
...
/// `?`-propagate failures into `TakutoError`. Production code paths never
...
#[cfg(test)]
impl From<rusqlite::Error> for TakutoError {
    fn from(e: rusqlite::Error) -> Self {
        TakutoError::Db(DbError::Sqlite(e))
    }
}

/// `?`-propagate adapter errors from DAOs using the agnostic
/// [`crate::db::DbAdapter`] API. The chain
/// `adapter::DbError → DbError::Adapter → TakutoError::Db` needs an
...
/// while preserving the existing `TakutoError::Db` envelope so logging
/// / HTTP-mapping code treats adapter errors identically to legacy
/// rusqlite errors.
impl From<crate::db::adapter::DbError> for TakutoError {
    fn from(e: crate::db::adapter::DbError) -> Self {
        TakutoError::Db(DbError::Adapter(e))
    }
}

pub type Result<T> = std::result::Result<T, TakutoError>;
