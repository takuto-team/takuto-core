...
/// passwordless `sudo bash` (granted in `/etc/sudoers.d/takuto-hook-bash`).
...
/// Takuto's background service; when it does not exist (local dev /
...
pub(crate) const GIT_CRED_HELPER_SNIPPET: &str = r#"git config --global 'credential.https://github.com.helper' '!f() { echo protocol=https; echo host=github.com; echo username=x-access-token; echo "password=${GH_TOKEN:-$(cat /home/takuto/.config/gh/gh-app-token 2>/dev/null)}"; }; f' 2>/dev/null || true;"#;

/// Shell snippet that sources every `/run/takuto-secrets/*` file into the
...
///
/// The snippet is self-gated on `TAKUTO_AUTH_BUNDLE=1` so it is a no-op
...
pub(crate) const BUNDLE_SOURCING_SH: &str = concat!(
    r#"if [ "${TAKUTO_AUTH_BUNDLE:-0}" = "1" ] && [ -d /run/takuto-secrets ]; then"#,
    ...
    r#" __bundle_present=$(ls -A /run/takuto-secrets 2>/dev/null | wc -l);"#,
    r#" if [ "${__bundle_present:-0}" = "0" ]; then"#,
    r#" echo "[takuto-bundle] TAKUTO_AUTH_BUNDLE=1 but /run/takuto-secrets/ is empty -- secret files vanished (host TempDir dropped). Check WorkerSecretsBundle lifetime in AppState." >&2;"#,
    r#" fi;"#,
    r#" if [ -f /run/takuto-secrets/claude ]; then"#,
    r#" CLAUDE_CODE_OAUTH_TOKEN="$(cat /run/takuto-secrets/claude)";"#,
    r#" export CLAUDE_CODE_OAUTH_TOKEN;"#,
    r#" rm -f /run/takuto-secrets/claude 2>/dev/null || true;"#,
    r#" fi;"#,
    r#" if [ -f /run/takuto-secrets/cursor ]; then"#,
    r#" CURSOR_API_KEY="$(cat /run/takuto-secrets/cursor)";"#,
    r#" export CURSOR_API_KEY;"#,
    r#" rm -f /run/takuto-secrets/cursor 2>/dev/null || true;"#,
    r#" fi;"#,
    r#" if [ -f /run/takuto-secrets/codex ]; then"#,
    r#" OPENAI_API_KEY="$(cat /run/takuto-secrets/codex)";"#,
    r#" export OPENAI_API_KEY;"#,
    r#" rm -f /run/takuto-secrets/codex 2>/dev/null || true;"#,
    r#" fi;"#,
    // OpenCode self-hosted spec (lore/audits/2026-05-27-opencode-self-hosted-spec.md):
    // No /run/takuto-secrets/opencode handling — the provider config is
    // a file, not an env var. The bundle's opencode_config_dir is staged
    // read-only at /run/takuto-opencode-config and copied into the
    ...
    r#" if [ -f /run/takuto-opencode-config/opencode.json ]; then"#,
    r#" mkdir -p "$HOME/.config/opencode";"#,
    r#" cp /run/takuto-opencode-config/opencode.json "$HOME/.config/opencode/opencode.json" || true;"#,
    ...
    r#" if [ -f /run/takuto-secrets/claude_session.json ]; then"#,
    r#" if [ -f "$HOME/.claude.json" ] && command -v jq >/dev/null 2>&1; then"#,
    r#" __mtmp=$(mktemp);"#,
    r#" if jq -s '.[0] * .[1]' "$HOME/.claude.json" /run/takuto-secrets/claude_session.json > "$__mtmp" 2>/dev/null; then"#,
    r#" mv "$__mtmp" "$HOME/.claude.json";"#,
    r#" else"#,
    r#" rm -f "$__mtmp";"#,
    r#" cp /run/takuto-secrets/claude_session.json "$HOME/.claude.json" || true;"#,
    r#" fi;"#,
    r#" else"#,
    r#" cp /run/takuto-secrets/claude_session.json "$HOME/.claude.json" || true;"#,
    r#" fi;"#,
    r#" rm -f /run/takuto-secrets/claude_session.json 2>/dev/null || true;"#,
    r#" fi;"#,
    r#" if [ -f /run/takuto-secrets/gh ]; then"#,
    r#" GH_TOKEN="$(cat /run/takuto-secrets/gh)";"#,
    r#" export GH_TOKEN;"#,
    r#" rm -f /run/takuto-secrets/gh 2>/dev/null || true;"#,
    r#" fi;"#,
    r#" fi"#,
);
...
mod tests {
    ...
    /// `/run/takuto-secrets/<file>` → env-var mapping documented in
    ...
    fn bundle_sourcing_snippet_covers_every_documented_mapping() {
        ...
        assert!(
            BUNDLE_SOURCING_SH.contains(r#"if [ "${TAKUTO_AUTH_BUNDLE:-0}" = "1" ]"#),
            "snippet must self-gate on TAKUTO_AUTH_BUNDLE=1"
        );
        ...
        for (file, env_var) in [
            ("/run/takuto-secrets/claude", "CLAUDE_CODE_OAUTH_TOKEN"),
            ("/run/takuto-secrets/cursor", "CURSOR_API_KEY"),
            ("/run/takuto-secrets/codex", "OPENAI_API_KEY"),
            ("/run/takuto-secrets/gh", "GH_TOKEN"),
        ] {
            assert!(
        ...
        assert!(
            !BUNDLE_SOURCING_SH.contains("/run/takuto-secrets/opencode"),
            "snippet must NOT source /run/takuto-secrets/opencode — \
        ...
        assert!(
            BUNDLE_SOURCING_SH.contains("[ -f /run/takuto-secrets/claude_session.json ]"),
        ...
        assert!(
            BUNDLE_SOURCING_SH
                .contains(r#"cp /run/takuto-secrets/claude_session.json "$HOME/.claude.json""#),
            "snippet must keep a cp fallback for the no-jq / no-existing-file case"
        );
        assert!(
            BUNDLE_SOURCING_SH.contains("rm -f /run/takuto-secrets/claude_session.json"),
            "snippet must rm -f /run/takuto-secrets/claude_session.json after merge"
        );

        // Observability breadcrumb. When TAKUTO_AUTH_BUNDLE=1 but the
        ...
        assert!(
            BUNDLE_SOURCING_SH.contains("[takuto-bundle]"),
            "snippet must carry the [takuto-bundle] stderr tag for the \
    ...
    /// the same `/run/takuto-secrets/<file>` ↔ env-var mappings the
    ...
    fn bundle_sourcing_matches_worker_entrypoint_shell_script() {
        // CARGO_MANIFEST_DIR for takuto-core is crates/takuto-core; the
        ...
        // "no /run/takuto-secrets/opencode env-var sourcing" rule.
        assert!(
            !script.contains("/run/takuto-secrets/opencode"),
        ...
        assert!(
            script.contains("/run/takuto-secrets/claude_session.json"),
        ...
        assert!(
            BUNDLE_SOURCING_SH.contains("/run/takuto-secrets/claude_session.json"),
        ...
        assert!(
            script.contains("[takuto-bundle]"),
            "drift: worker-entrypoint.sh missing [takuto-bundle] empty-mountpoint warning"
        );
        assert!(
            BUNDLE_SOURCING_SH.contains("[takuto-bundle]"),
            "drift: BUNDLE_SOURCING_SH missing [takuto-bundle] empty-mountpoint warning"
        );
    }

    /// When the runner has NO secrets bundle attached, the
    /// `sh -c` payload built by `build_sh_payload` must NOT reference
    /// `/run/takuto-secrets/` — keeps the legacy path's argv clean and
    /// avoids any chance of confusing logs.
    #[test]
    fn build_sh_payload_without_bundle_does_not_source_run_takuto_secrets() {
        let cmd = build_sh_payload(false, "claude", &["--version"]);
        assert!(
            !cmd.contains("/run/takuto-secrets/"),
            "legacy payload must not reference /run/takuto-secrets/; got: {cmd}"
        );
        assert!(
            !cmd.contains("TAKUTO_AUTH_BUNDLE"),
            "legacy payload must not gate on TAKUTO_AUTH_BUNDLE; got: {cmd}"
        );
    ...
    fn build_sh_payload_with_bundle_sources_secrets_before_exec() {
        ...
        assert!(
            cmd.contains("/run/takuto-secrets/claude"),
            "bundle-attached payload must source /run/takuto-secrets/claude; got: {cmd}"
        );
        assert!(
        ...
        let bundle_pos = cmd
            .find("/run/takuto-secrets/claude")
        ...
        for file in [
            "/run/takuto-secrets/claude",
            "/run/takuto-secrets/cursor",
            "/run/takuto-secrets/codex",
            "/run/takuto-secrets/gh",
        ] {
            assert!(
                cmd.contains(file),
    ...
    fn git_cred_helper_prefers_gh_token_over_app_file() {
        assert!(
            GIT_CRED_HELPER_SNIPPET.contains(
                r#"${GH_TOKEN:-$(cat /home/takuto/.config/gh/gh-app-token 2>/dev/null)}"#
            ),
