...
pub async fn start_run_command(
    ...
    // Caller-provided environment (e.g. TAKUTO_PROXY_BASE for dev servers).
    ...
    args.push(format!("takuto.run_command={cmd_index}"));
    args.push("--label".into());
    args.push(format!("takuto.ticket_key={ticket_key}"));
    ...
    let script =
        format!("[ -f /etc/takuto/env ] && set -a && . /etc/takuto/env && set +a; {command}");
    args.push("bash".into());
    args.push("-lc".into());
    args.push(script);
...
pub async fn run_run_command_port_scanner(
    ...
    // Takuto dashboard, etc.) and must not be forwarded. Only ports that
