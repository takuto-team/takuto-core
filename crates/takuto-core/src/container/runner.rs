...
pub struct ContainerRunner {
    ...
    /// for the worktree, `.git`, and `.takuto` — see `super::volumes`.
    ...
    /// tmpfs at `/run/takuto-secrets:ro` and `super::wrap_command`
...
impl ContainerRunner {
    ...
    /// `/workspace` volume, only the worktree directory, `.git`, and `.takuto`
    ...
    /// bundle's tmpfs at `/run/takuto-secrets:ro`, sets
    /// `TAKUTO_AUTH_BUNDLE=1`, and exports the bundle's non-secret env
    ...
    pub fn next_container_name(&self) -> String {
        ...
        format!("takuto-worker-{sanitized}-{n}")
    }
    ...
    pub fn wrap_command(&self, program: &str, args: &[&str]) -> (String, Vec<String>) {
        ...
        // root-owned files on the bind-mounted worktree that takuto can't remove.
        docker_args.push("--user".into());
        docker_args.push("takuto:takuto".into());
    ...
    /// Auto-detect the worker image by inspecting the running Takuto container,
    /// falling back to a locally-present `takuto:latest`, then `TAKUTO_REGISTRY_IMAGE`.
    pub async fn discover_worker_image() -> Option<String> {
            ...
            && output.status.success()
        {
            ...
            if !image.is_empty() && image_exists_in_dind(&image).await {
                info!(image = %image, "Discovered worker image from running Takuto container");
                return Some(image);
            }
        }

        // takuto:latest in DinD (e.g. via `make load-worker`) — local dev builds.
        if image_exists_in_dind("takuto:latest").await {
            info!("Using local takuto:latest as worker image");
            return Some("takuto:latest".to_string());
        }

        // Fall back to TAKUTO_REGISTRY_IMAGE (set at build time in the Dockerfile)
        if let Ok(image) = std::env::var("TAKUTO_REGISTRY_IMAGE")
            && !image.is_empty()
        {
            info!(image = %image, "Using TAKUTO_REGISTRY_IMAGE as worker image");
            return Some(image);
        }

        warn!(
            "Cannot auto-detect worker image — docker inspect failed, takuto:latest not found, and TAKUTO_REGISTRY_IMAGE not set"
        );
        None
    }
}
...
mod tests {
    ...
    fn next_container_name_increments() {
        let r = runner();
        assert_eq!(r.next_container_name(), "takuto-worker-proj-42-0");
        assert_eq!(r.next_container_name(), "takuto-worker-proj-42-1");
        assert_eq!(r.next_container_name(), "takuto-worker-proj-42-2");
    }
    ...
    fn wrap_command_structure() {
        ...
        // Container name
        assert_eq!(flag_value(&args, "--name"), Some("takuto-worker-proj-42-0"));

        // NET_ADMIN
        assert!(args.contains(&"--cap-add=NET_ADMIN".to_string()));

        // Key env vars
        assert!(has_env(&args, "HOME", "/home/takuto"));
        ...
        assert!(has_volume(&args, "/shared-auth/gh:/home/takuto/.config/gh"));
        ...
        assert_eq!(flag_value(&args, "--user"), Some("takuto:takuto"));

        // After --entrypoint "" comes: --user takuto:takuto, image, sh, -c, "restore; exec ..."
        let entrypoint_idx = args.iter().position(|a| a == "--entrypoint").unwrap();
        let tail = &args[entrypoint_idx + 2..];
        assert_eq!(tail[0], "--user");
        assert_eq!(tail[1], "takuto:takuto");
        assert_eq!(tail[2], "takuto:latest");
        assert_eq!(tail[3], "sh");
        assert_eq!(tail[4], "-c");
    ...
    fn wrap_shell_command_uses_worker_entrypoint() {
        ...
        let entrypoint_idx = args.iter().position(|a| a == "--entrypoint").unwrap();
        let tail = &args[entrypoint_idx + 2..];
        assert_eq!(tail[0], "takuto:latest");
        assert_eq!(tail[1], "sh");
        assert_eq!(tail[2], "-c");
    ...
    fn wrap_command_counter_advances_across_calls() {
        ...
        assert_eq!(
            ...
            Some("takuto-worker-proj-42-0")
        );
        assert_eq!(
            flag_value(&args2, "--name"),
            Some("takuto-worker-proj-42-1")
        );
    }
    ...
    #[test]
    fn isolated_mode_has_takuto_dir_mount_ro() {
        let r = isolated_runner();
        let (_, args) = r.wrap_command("true", &[]);
        assert!(
            has_volume(&args, "/workspace/.takuto:/workspace/.takuto:ro"),
            "Isolated mode must mount .takuto read-only for npm config"
        );
    }
    ...
    fn isolated_mode_auth_volumes_preserved() {
        ...
        for mount in super::super::volumes::WORKER_VOLUMES {
            if mount.starts_with("/shared-auth/") || mount.starts_with("/etc/takuto") {
                assert!(
    ...
    fn isolated_wrap_shell_command_has_targeted_mounts() {
        ...
        assert!(
            has_volume(&args, "/workspace/.takuto:/workspace/.takuto:ro"),
            "Isolated wrap_shell_command must mount .takuto:ro"
        );
    }
