...
//!
//! When `connection` is empty (or the section is omitted entirely), Takuto
//! keeps the legacy "SQLite at {data_dir}/takuto.db" behaviour. Setting it
...
pub struct DatabaseConfig {
    ...
    /// When true and a local SQLite file exists at `{data_dir}/takuto.db`
...
mod tests {
    ...
    fn redact_postgres_password() {
        assert_eq!(
            redact_connection_password("postgres://takuto:s3cret@db.local:5432/takuto"),
            "postgres://takuto:****@db.local:5432/takuto"
        );
    }
    ...
    fn redact_keeps_url_without_password() {
        assert_eq!(
            redact_connection_password("postgres://takuto@db.local/takuto"),
            "postgres://takuto@db.local/takuto"
        );
    }
    ...
    fn redact_passes_through_sqlite_url() {
        assert_eq!(
            redact_connection_password("sqlite:///var/lib/takuto/takuto.db"),
            "sqlite:///var/lib/takuto/takuto.db"
        );
    }
