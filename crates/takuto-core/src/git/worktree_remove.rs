...
//! leaving root-owned files on bind-mounted worktrees. The main Takuto process then could not
//! delete those paths. We retry after a non-interactive `sudo bash -c chown` when Git reports
//! permission denied (Takuto image sudoers allow this).
...
/// E.g., `/Users/user/dev/takuto` → `.claude/projects/-Users-user-dev-takuto`
...
mod tests {
    ...
    fn compute_session_folder_encodes_path_correctly() {
        let repo_path = Path::new("/Users/alexanderobellianne/dev/takuto");
        let session_folder = compute_session_folder(repo_path);

        // Should contain the encoded path
        let path_str = session_folder.to_str().unwrap();
        assert!(path_str.contains(".claude/projects"));
        assert!(path_str.contains("-Users-alexanderobellianne-dev-takuto"));
    }
}
