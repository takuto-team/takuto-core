...
//!
//! Replaces the historical `TakutoError::Git(String)` (the `*Str(String)`
...
//! `?` propagation because they previously wrapped an inner `TakutoError` in
...
mod tests {
    ...
    #[test]
    fn lock_in_git_error_into_takuto_error() {
        ...
        for err in cases {
            ...
            assert!(
                matches!(outer, TakutoError::Git(_)),
                "expected TakutoError::Git, got {outer:?}"
            );
        }
    }
}
