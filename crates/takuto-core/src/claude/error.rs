...
//! `crates/takuto-core/src/claude/`. Lifted from `TakutoError::Claude(String)`
...
//! `TakutoError::Claude(#[from] ClaudeError)` so existing `?` propagation
//! across `Result<T, TakutoError>` boundaries keeps working unchanged.
...
mod tests {
    ...
    //!   2. The `#[from] ClaudeError` chain into `TakutoError::Claude(..)` —
    //!      every `?`-propagation inside `crates/takuto-core/src/claude/`
    ...
    #[test]
    fn lock_in_claude_error_into_takuto_error() {
        let non_zero = ClaudeError::NonZeroExit {
            exit_code: 2,
            detail: "(no output)".to_string(),
        };
        let wrapped: TakutoError = non_zero.into();
        assert!(matches!(
            wrapped,
            TakutoError::Claude(ClaudeError::NonZeroExit { .. })
        ));

        let empty = ClaudeError::EmptyOutput;
        let wrapped: TakutoError = empty.into();
        assert!(matches!(
            wrapped,
            TakutoError::Claude(ClaudeError::EmptyOutput)
        ));
    }
}
