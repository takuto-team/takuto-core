...
/// `takuto preflight` CLI subcommand) pass `None` and get config-only
/// warnings.
...
/// directory*, not the target file — so if the bind-mounted `/etc/takuto/`
/// is root-owned and the runtime user is `takuto`, every dashboard save
        ...
        .tempfile_in(dir)
    {
        ...
        Err(e) if e.kind() == std::io::ErrorKind::PermissionDenied => {
            Some(StructuredWarning::critical(
                ...
                format!(
                    "Takuto can't write to the config directory ({}). Dashboard \
                     configuration changes will not persist across restarts. \
                     Restart Takuto after fixing directory permissions.",
                    dir.display()
                ),
            ))
        }
...
pub fn collect_system_status_with_db(
        ...
        match db.master_key() {
            None => {
                warnings.push(StructuredWarning::critical(
                    ...
                    "Master key unavailable: set TAKUTO_SECRET_KEY or enable [general] allow_auto_generate_secret_key. Per-user credential storage is disabled until this is resolved.",
                ));
            }
    ...
    let provider = match config.agent.provider {
        ...
        // PATH in the worker image (both are baked into the takuto
        // image today).
    ...
    SystemStatus {
        ...
        // is available (see `takuto-cli` `run_server`). Default `false`
...
mod system_status_tests {
    ...
    fn check_config_dir_writable_returns_none_when_parent_missing() {
        let cfg_path = std::path::Path::new("/nonexistent/takuto-task-37/parent/config.toml");
        let result = check_config_dir_writable(cfg_path);
        assert!(
            result.is_none(),
