...
//! `GitAuthResolver` — orchestration over the per-user GitHub auth mode.
//!
//! Picks the right token (GitHub App installation token vs. per-user PAT)
//! for a given `GitAction`, materialising the token from the right backend.
//! Pure decision logic lives in [`super::decision`]; PAT/SSO revalidation
//! lives in [`super::validator`]; audit writes live in [`super::audit`];
//! the error / secret types live in [`super::errors`]. This file is the
//! glue.
//!
//! The struct is `Clone`-cheap because both backend handles are `Arc`-shared.
...
/// the parent module. Holds a [`Database`] handle (for the user PAT row)
/// and an optional [`GitHubAppTokenManager`] (for installation tokens). The
...
impl GitAuthResolver {
    ...
    /// [`super::validator`] for the `credential_audit::log` write on
    /// validation failure. Crate-external callers must not depend on this —
    /// the resolver's public surface is `mode_for_user` / `token_for` /
    /// `revalidate_*`.
    ...
    pub(super) async fn unseal_user_pat(&self, user_id: &str) -> GitAuthResult<SecretToken> {
        let mk = self
            .db
            .master_key()
            .ok_or_else(|| GitAuthError::MasterKeyUnavailable {
                user_id: user_id.to_string(),
            })?
            .key
            .clone();
...
mod tests {
    ...
    async fn revalidate_sso_returns_sso_required_when_pat_blocked() {
        ...
        assert_eq!(
            GitAuthError::SsoAuthorizationRequired {
                org: "x".into(),
                sso_url: "y".into(),
            }
            .code(),
