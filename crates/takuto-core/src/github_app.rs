...
//!
//! When configured, Takuto uses a GitHub App installation token instead of the
//! personal `gh` CLI credentials. Commits and PRs are attributed to the App's
//! bot identity (e.g. `takuto-bot[bot]`).
//!
//! # Authentication flow
//!
...
/// both the main Takuto container and worker containers.
pub const TOKEN_FILE_PATH: &str = "/home/takuto/.config/gh/gh-app-token";
...
impl GitHubAppTokenManager {
    ...
    /// Git author name for the bot (e.g. `takuto-bot[bot]`).
    pub fn bot_name(&self) -> &str {
        "takuto-bot[bot]"
    }

    /// Git author email for the bot (e.g. `123456+takuto-bot[bot]@users.noreply.github.com`).
    pub fn bot_email(&self) -> String {
        format!("{}+takuto-bot[bot]@users.noreply.github.com", self.app_id)
    }
    ...
    ///
    /// The task runs until `cancel` is triggered (i.e. Takuto shutdown).
...
mod tests {
    ...
    fn bot_name_is_fixed() {
        assert_eq!(mgr_with_app_id(0).bot_name(), "takuto-bot[bot]");
    }
    ...
    fn bot_email_contains_app_id() {
        ...
        assert_eq!(
            mgr.bot_email(),
            "123456+takuto-bot[bot]@users.noreply.github.com"
        );
    }
