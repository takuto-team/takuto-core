...
pub(super) async fn persist_new_work_item(
    db: Option<&Database>,
    workflow: &Workflow,
) -> Result<()> {
    let Some(db) = db else { return Ok(()) };
    let row = workflow.to_work_item_row();
    super::driver::retry_durable_write(|| {
        crate::db::work_items::insert_work_item(db.adapter(), &row)
    })
    .await
    .inspect_err(|e| {
        error!(
...
mod facade_engine_tests {
    ...
    async fn start_workflow_propagates_user_id() {
        ...
        let jira_available = Arc::new(AtomicBool::new(false));
        let workflows_dir =
            std::env::temp_dir().join(format!("takuto-engine-test-{}", uuid::Uuid::new_v4()));
    ...
    async fn shadow_def_run_start_and_finish_round_trip() {
        ...
        crate::workflow::engine::driver::shadow_start_def_run(
            Some(&db),
            "wf-def-1",
            "ship.toml",
            200,
