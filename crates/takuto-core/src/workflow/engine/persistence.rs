...
mod tests {
    ...
    /// Serializes the few tests that mutate `TAKUTO_DATA_DIR`, since the env
    ...
    async fn restore_marks_renamed_flow_as_no_longer_existing() {
        ...
        // test in the crate that sets TAKUTO_DATA_DIR.
        unsafe {
            std::env::set_var("TAKUTO_DATA_DIR", data_dir.path());
        }
    ...
    async fn restore_rebuilds_from_db_with_no_snapshot_file() {
        ...
        assert!(
            in_map,
            "must rebuild TICK-3 from the DB with no snapshot file"
        );
