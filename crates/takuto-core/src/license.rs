...
/// License tiers available for Takuto.
...
/// Initialise the license tier from the `TAKUTO_LICENSE_TIER` environment variable.
///
...
pub fn init_license_tier() {
    let raw = std::env::var("TAKUTO_LICENSE_TIER").unwrap_or_else(|_| "community".to_string());
    let Some(tier) = LicenseTier::from_str_value(&raw) else {
        tracing::error!(
            "Invalid TAKUTO_LICENSE_TIER value: \"{raw}\". \
