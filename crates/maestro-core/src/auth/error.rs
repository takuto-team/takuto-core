...
pub enum AuthError {
    ...
    // ── Web routes (`maestro-web/src/routes/{auth,admin}.rs`) ─────────────
    ...
    // ── Web auth middleware (`maestro-web/src/auth.rs`) ───────────────────
...
mod tests {
    ...
    #[test]
    fn lock_in_auth_error_into_maestro_error() {
        ...
        for err in cases {
            ...
            assert!(
                matches!(outer, MaestroError::Auth(_)),
                "expected MaestroError::Auth, got {outer:?}"
            );
        }
    }
}
