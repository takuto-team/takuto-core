...
//! 2. `MAESTRO_DEV_MOCK_AGENT` env var (`1` / `true` / `TRUE` / `yes`).
...
fn env_enabled() -> bool {
    std::env::var("MAESTRO_DEV_MOCK_AGENT")
        .ok()
        .as_deref()
...
mod tests {
    ...
    fn clear_env() {
        ...
        unsafe {
            std::env::remove_var("MAESTRO_DEV_MOCK_AGENT");
        }
    }
    ...
    fn env_var_enables() {
        ...
        unsafe {
            std::env::set_var("MAESTRO_DEV_MOCK_AGENT", "1");
        }
        assert!(is_enabled_from_runtime());
        clear_env();
        assert!(!is_enabled_from_runtime());
    ...
    fn env_var_accepts_yes_and_true() {
        ...
        for v in ["1", "true", "TRUE", "yes"] {
            unsafe {
                std::env::set_var("MAESTRO_DEV_MOCK_AGENT", v);
            }
            assert!(is_enabled_from_runtime(), "expected enabled for {v}");
        }
        unsafe {
            std::env::set_var("MAESTRO_DEV_MOCK_AGENT", "0");
        }
        assert!(!is_enabled_from_runtime());
        clear_env();
    }
    ...
    async fn claude_mock_honors_cancel() {
        ...
        assert!(
            matches!(res, Err(MaestroError::Cancelled)),
