...
//! Typed errors for what `MaestroError::Config(String)` had become — a true
...
pub enum ConfigError {
    ...
    /// `MAESTRO_SECRET_KEY` env var is not valid hex.
    #[error("MAESTRO_SECRET_KEY is not valid hex: {source}")]
    MasterKeyHex {
        #[source]
        source: hex::FromHexError,
    },

    /// `MAESTRO_SECRET_KEY` decoded to the wrong number of bytes.
    #[error("MAESTRO_SECRET_KEY decoded to wrong length")]
    MasterKeyLength,
...
mod tests {
    ...
    fn lock_in_config_error_display_samples() {
        ...
        let cases: Vec<(ConfigError, String)> = vec![
            ...
            (ConfigError::MasterKeyLength, "MAESTRO_SECRET_KEY decoded to wrong length".to_string()),
            (
    ...
    #[test]
    fn lock_in_config_error_into_maestro_error_all_variants() {
        ...
        for err in cases {
            ...
            assert!(
                matches!(outer, MaestroError::Config(_)),
                "expected MaestroError::Config, got {outer:?}"
            );
        }
    }
}
