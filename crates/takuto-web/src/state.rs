...
pub type RunCommandBundlesMap =
    Arc<RwLock<HashMap<(String, usize), Arc<takuto_core::auth::WorkerSecretsBundle>>>>;
...
pub struct EngineState {
    ...
    pub system_status: Arc<RwLock<takuto_core::docker_hooks::SystemStatus>>,
}
...
pub struct AuthState {
    ...
    pub db: Option<takuto_core::db::Database>,
    /// Injectable shim around the `gh` CLI for per-user PAT validation.
    /// Production uses [`takuto_core::auth::RealGhClient`]; tests inject a
    /// `MockGhClient` so the suite never touches github.com.
    pub gh_client: takuto_core::auth::SharedGhClient,
...
pub struct ConfigState {
    ...
    pub work_item_flow_defaults: Arc<Vec<takuto_core::db::user_work_item_flows::UserFlow>>,
}
...
impl ConfigState {
    ...
    pub async fn active_workspace_name(&self) -> String {
        ...
        takuto_core::workflow::snapshot::workspace_name_from_repo_path(std::path::Path::new(
            &repo_path,
        ))
    }
}
...
pub struct EditorState {
    ...
    /// lifetime so the bind-mounted `/run/takuto-secrets/` stays populated.
    ...
    pub editor_bundles: Arc<RwLock<HashMap<String, Arc<takuto_core::auth::WorkerSecretsBundle>>>>,
