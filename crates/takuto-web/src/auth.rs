...
use takuto_core::config::WebConfig;
...
pub const SESSION_COOKIE_NAME: &str = "takuto_session";
...
pub async fn create_db_session(
    ...
    user_id: &str,
) -> std::result::Result<String, TakutoError> {
...
pub async fn delete_db_session(adapter: &takuto_core::db::DbAdapter, cookie_value: &str) -> bool {
    use takuto_core::db::DbValue;
...
///
/// Returns the authenticated [`User`](takuto_core::db::models::User) on success,
/// or `None` if credentials are invalid or the user is suspended.
pub async fn authenticate_db_user(
    db: &Database,
    username: &str,
    password: &str,
) -> Option<takuto_core::db::models::User> {
    let adapter = db.adapter();
    let user = match takuto_core::db::users::get_user_by_username(adapter, username).await {
        Ok(Some(u)) => u,
        _ => return None,
    };
    if user.suspended {
    ...
    match takuto_core::db::credentials::verify_user_password(adapter, &user.id, password).await {
        Ok(true) => Some(user),
        _ => None,
    }
}
...
pub struct AuthenticatedUser {
    pub user_id: String,
    pub role: takuto_core::db::models::UserRole,
}
...
pub async fn dashboard_auth_middleware(
        ...
        && raw_cookie.starts_with(DB_SESSION_PREFIX)
    {
        ...
        let auth_user = if let Some(uid) = user_id {
            takuto_core::db::users::get_user_by_id(adapter, &uid)
                .await
                .ok()
                .flatten()
...
mod tests {
    ...
    fn session_cookie_from_headers_extracts_value() {
        ...
        headers.insert(
            COOKIE,
            "other=x; takuto_session=abc123.def456; foo=bar"
                .parse()
                .unwrap(),
        );
    ...
    fn resolve_cookie_secure_https_cors_origin_triggers_true() {
        let web = WebConfig {
            cors_origins: vec!["https://takuto.example.com".into()],
            ..Default::default()
        };
        let headers = HeaderMap::new();
        assert!(resolve_cookie_secure(&web, &headers));
