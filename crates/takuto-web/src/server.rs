...
///
/// Prefers `X-Forwarded-For` / `X-Real-IP` (Takuto normally runs behind a
...
pub fn build_router(state: AppState) -> Router {
    ...
    let api_protected = Router::new()
        ...
        // External callers get a `X-Takuto-Deprecation` header on the
    ...
    Router::new()
        ...
        // `X-Takuto-Deprecation` so external callers know to switch to
...
pub fn build_cors_layer(web_config: &takuto_core::config::WebConfig) -> CorsLayer {
