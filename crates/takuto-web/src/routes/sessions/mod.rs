...
//!    `takuto_session` cookie (database-backed session). Unauthenticated
...
/// Strategy 2: `takuto_dynamic_port` cookie → set when the user last
...
/// requires a valid `takuto_session` cookie before proceeding. This prevents
    ...
    async fn proxy_returns_401_when_db_present_with_invalid_cookie() {
        ...
        let resp = app
            .oneshot(
                Request::get("/s/aaaa1111bbbb2222cccc3333dddd4444/")
                    .header("Host", "localhost")
                    .header("Cookie", "takuto_session=db-nonexistent-session-id")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
