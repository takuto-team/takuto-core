...
/// mode the takuto container shares the DinD container's network
...
pub(super) fn upstream_path_for_kind(kind: SessionRouteKind, token: &str, rest: &str) -> String {
    match kind {
        ...
        // (e.g. Vite) can use TAKUTO_PROXY_BASE; apps that don't (e.g.
        ...
        Ok(resp) => {
            ...
            if route.kind == SessionRouteKind::DynamicPort
                ...
                && let Ok(cookie_val) = HeaderValue::from_str(&format!(
                    "takuto_dynamic_port={token}; Path=/; SameSite=Lax"
                ))
            {
                response
                    .headers_mut()
