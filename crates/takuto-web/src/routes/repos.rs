...
mod ts_bindings {
    ...
    /// CI diffs the directory; see `crates/takuto-web/src/ts_bindings.rs`.
    #[test]
...
pub async fn list_github_repos(
    ...
    if !output.success() {
        return Err((
            ...
            format!(
                "GitHub API error: {}",
                takuto_core::github::gh_api_error_message(output.stderr.trim(), "Metadata: Read")
            ),
        ));
    }
...
pub(crate) async fn do_clone(
    ...
    let gh_token = match (user_id, auth_state.git_auth_resolver.as_ref()) {
        (Some(uid), Some(resolver)) => match resolver.token_for(GitAction::Clone, uid).await {
            ...
            Err(takuto_core::github::auth_resolver::GitAuthError::UnauthenticatedGit {
                ..
            }) => {
                return Err(
    ...
    if let Some(token) = gh_token {
        ...
        let output = tokio::time::timeout(
            CLONE_TIMEOUT,
            takuto_core::process::run_command_with_env(
                "git",
                &[
                    "-c",
        ...
        let output = tokio::time::timeout(
            CLONE_TIMEOUT,
            takuto_core::process::run_command(
                "gh",
    ...
    let _ = takuto_core::process::run_command(
        "git",
