...
pub async fn register(
    ...
    let result: takuto_core::error::Result<RegisterResponse> = async {
        let count = takuto_core::db::users::count_users(adapter).await?;
        if count > 0 {
        ...
        let user = takuto_core::db::users::create_user(
            adapter,
            &username,
            takuto_core::db::models::UserRole::Admin,
        )
        .await?;
