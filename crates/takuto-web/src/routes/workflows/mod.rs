...
pub(crate) async fn require_workflow_access(
    ...
    if let Some(database) = auth_state.db.as_ref() {
        match takuto_core::db::work_items::get_access_fields_by_ticket_key(
            database.adapter(),
            ticket_key,
        )
        .await
        {
            Ok(Some((owner, repo_id, workspace))) => {
                ...
                let repos = match takuto_core::db::repositories::list_for_user(
                    database.adapter(),
                    &auth.user_id,
                )
                .await
...
pub(super) async fn build_editor_or_run_command_bundle(
    ...
    user_id: &str,
) -> Option<std::sync::Arc<takuto_core::auth::WorkerSecretsBundle>> {
    ...
    let result = match pin {
        Some(pin) => {
            takuto_core::auth::bundle::build(&cfg_snapshot, db, resolver, &pin, user_id).await
        }
        None => {
            takuto_core::auth::bundle::build_for_endpoint(&cfg_snapshot, db, resolver, user_id)
                .await
        }
    };
    match result {
