...
async fn create_regular_user_login(
    state: &takuto_web::state::AppState,
    admin_cookie: &str,
) -> String {
...
async fn put_config_agent_as_admin_updates_and_persists() {
    ...
    let dir = std::env::temp_dir().join(format!("takuto-config-agent-{}", uuid::Uuid::new_v4()));
...
async fn put_config_agent_emits_config_file_bind_mounted_when_writer_flag_set() {
    ...
    let dir = std::env::temp_dir().join(format!("takuto-task-38-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).unwrap();
    let config_path = dir.join("config.toml");
    std::fs::write(&config_path, "").unwrap();
