...
use takuto_core::config::SkillRef;
use takuto_core::db::user_work_item_flows::{UserFlow, UserFlowStep};
...
async fn flows_are_isolated_per_workspace() {
    ...
    switch_workspace(&state, "/tmp/takuto-ws-alpha").await;
    ...
    switch_workspace(&state, "/tmp/takuto-ws-bravo").await;
    assert_eq!(
        get_flow_names(&state, &cookie).await,
        vec!["Implement Ticket", "Review"],
