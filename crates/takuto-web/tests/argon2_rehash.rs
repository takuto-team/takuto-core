...
// `takuto_core::db::credentials` so this test file does not need to depend on
...
use takuto_core::db::credentials::legacy_argon2_default_hash_for_tests;
...
async fn admin_password_credential(db: &takuto_core::db::Database) -> (String, String) {
    let row = db
        .adapter()
        .query_one(
...
async fn credential_data(db: &takuto_core::db::Database, cred_id: &str) -> Vec<u8> {
    let row = db
        .adapter()
        .query_one(
