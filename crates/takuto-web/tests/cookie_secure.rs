...
//
// Resolution rule (from `crates/takuto-web/src/auth.rs::resolve_cookie_secure`):
...
/// Extract the `Set-Cookie` header for `takuto_session` from the response.
...
fn session_set_cookie(resp: &axum::http::Response<Body>) -> String {
    for v in resp.headers().get_all("set-cookie").iter() {
        ...
        if s.starts_with("takuto_session=") {
            return s.to_string();
        }
    }
    panic!("no takuto_session Set-Cookie header on response");
}
...
async fn cookie_secure_when_https_origin_present() {
    ...
    {
        let mut cfg = state.config().config.write().await;
        cfg.web.cors_origins = vec![TEST_ORIGIN.into(), "https://takuto.example.com".into()];
    }
