...
//
// Background: the editor terminal showed `/run/takuto-secrets/` was empty
...
async fn t_editor_bundle_temp_dir_outlives_route_handler_scope() {
    ...
    let resolver = state
        ...
        .clone();
    let bundle = takuto_core::auth::bundle::build_for_endpoint(
        &cfg_snapshot,
...
async fn t_run_command_bundle_map_is_keyed_by_ticket_and_index() {
    ...
    let mk_bundle = || async {
        let b = takuto_core::auth::bundle::build_for_endpoint(
            &cfg_snapshot,
            state.auth().db.as_ref().unwrap(),
            &resolver,
            user_id,
