...
async fn seed_provider_credential(state: &AppState, user_id: &str, provider: &str) {
    ...
    let mk = db
        ...
        .key
        .clone();
    let sealed = takuto_core::auth::seal(&mk, b"sk-test-token").unwrap();
    let adapter = db.adapter();
    let mut tx = adapter.begin().await.unwrap();
    takuto_core::db::provider_credentials::upsert(
        &mut tx,
        user_id,
        provider,
...
async fn seed_github_credential(state: &AppState, user_id: &str) {
    ...
    let sealed = takuto_core::auth::seal(&mk, b"ghp_test_pat").unwrap();
    let adapter = db.adapter();
    let mut tx = adapter.begin().await.unwrap();
...
async fn t_onb_filter_002_mismatched_credential_does_not_filter_active_warning() {
    ...
    seed_provider_credential(&state, &user_id, "claude").await;
    {
        ...
        cfg.agent.provider = takuto_core::config::AiAgentProvider::Cursor;
    }
