...
use takuto_web::test_helpers::{TEST_ORIGIN, register_and_login};
...
impl TempDir {
    fn new() -> Self {
        let p = std::env::temp_dir().join(format!("takuto-repos-it-{}", uuid::Uuid::new_v4()));
...
async fn seed_repository(
    ...
    takuto_core::db::repositories::upsert(db.adapter(), name, repo_url, local_path, "main", None)
        .await
...
async fn ac7_last_user_remove_purges_disk() {
    ...
    let row = takuto_core::db::repositories::get(db.adapter(), &r1)
        .await
        .unwrap();
...
async fn ac11_user_delete_cascades_to_associations() {
    ...
    let bob = takuto_core::db::users::get_user_by_username(db.adapter(), "bob")
        .await
        .unwrap()
        .unwrap();
    takuto_core::db::users::delete_user(db.adapter(), &bob.id)
        .await
        .unwrap();
    ...
    let assoc_count: i64 = db
        ...
        .query_one(
            ...
            vec![takuto_core::db::DbValue::Text(r1.clone())],
        )
        .await
        .unwrap()
        .get_i64(0)
...
async fn user_id_for(state: &AppState, username: &str) -> String {
    ...
    db.adapter()
        .query_one(
            ...
            vec![takuto_core::db::DbValue::Text(username.to_string())],
        )
        .await
...
async fn ac16_callers_active_workflow_blocks_delete() {
    ...
    let prev = std::env::var("TAKUTO_DATA_DIR").ok();
    ...
    // reset it after.
    unsafe {
        std::env::set_var("TAKUTO_DATA_DIR", tmp.path());
    }
    ...
    let snap = WorkflowSnapshotFile {
        ...
        workflows: vec![PersistedWorkflowRecord {
            ...
            ticketing_system: takuto_core::config::TicketingSystem::None,
            ticket_url: None,
    ...
    unsafe {
        match prev {
            Some(v) => std::env::set_var("TAKUTO_DATA_DIR", v),
            None => std::env::remove_var("TAKUTO_DATA_DIR"),
        }
    }
}
...
/// remove `takuto-core` and was 409'd because admin had an active workflow
/// on the repo.
#[tokio::test]
