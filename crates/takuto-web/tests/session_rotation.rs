...
//
// Time-dependent cases drive a test-only clock seam in `takuto_web::auth`
...
async fn sliding_extend_only_when_threshold_crossed() {
    ...
    let initial_last_seen: i64 = db
        ...
        .query_one(
            ...
            vec![takuto_core::db::DbValue::Text(alice_id.clone())],
        )
        .await
        .unwrap()
        .get_i64(0)
...
async fn absolute_ttl_rejects_and_deletes_old_session() {
    ...
    db.adapter()
        .execute(
            ...
            vec![takuto_core::db::DbValue::I64(ancient)],
        )
        .await
        .unwrap();
