...
use takuto_core::auth::gh_client::{GhClient, GhResponse};
...
async fn state_with_mock(gh: Arc<dyn GhClient>) -> (AppState, String) {
    let mut state = takuto_web::test_helpers::test_state_with_db();
...
fn break_master_key(state: &mut AppState) {
    let dir = std::env::temp_dir().join(format!("takuto-cred-degraded-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).unwrap();
    state.auth_mut().db =
        Some(takuto_core::db::Database::open(&dir, false).expect("open DB with auto-gen disabled"));
}
...
async fn t_jq_merge_001_session_overlay_preserves_existing_keys() {
    if std::process::Command::new("jq")
        ...
        .unwrap_or(true)
    {
        eprintln!("skip: jq not available on host; bundle uses it in the takuto image");
        return;
    }

    let dir = std::env::temp_dir().join(format!("takuto-t-41-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).unwrap();
...
async fn t_jq_merge_002_partial_paste_keeps_other_fields_intact() {
    ...
    let dir = std::env::temp_dir().join(format!("takuto-t-41b-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).unwrap();
    let existing = dir.join("existing.json");
    let bundle_blob = dir.join("bundle.json");
