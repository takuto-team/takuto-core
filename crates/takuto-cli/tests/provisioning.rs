...
//
// Integration tests for `takuto provisioning {sha,commands}`.
//
// These tests invoke the compiled `takuto` binary against a temp config
// file so the entrypoint shell's contract (run `takuto provisioning
// sha`, compare against a stored SHA file; run `takuto provisioning
...
// if its output shape changes, every Takuto deployment's boot would
// break, so we lock it down here.

use std::process::Command;

/// Locate the compiled `takuto` binary in the workspace target dir.
...
/// T-PROV-CLI-001: `takuto provisioning sha` prints a 64-char hex
...
fn t_prov_cli_003_sha_changes_when_command_changes() {
    ...
    let sha_a = Command::new(takuto_bin())
        .args(["--config"])
        .arg(&cfg_a)
        .args(["provisioning", "sha"])
        .output()
    ...
    let sha_b = Command::new(takuto_bin())
        .args(["--config"])
        .arg(&cfg_b)
        .args(["provisioning", "sha"])
        .output()
...
/// T-PROV-CLI-004: `takuto provisioning commands` emits NUL-separated
